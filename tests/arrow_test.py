import pytest
import io
from tests.utils import skip_if_no, CONSTANTS
import pyxet
...
def test_pyarrow_parquet():
    ...
    pf = ParquetFile(CONSTANTS.TITANIC_PARQUET, filesystem=pyxet.XetFS(url_info=CONSTANTS.TITANIC_MAIN))
...
def test_pyarrow_stream():
    ...
    pa_fs = PyFileSystem(FSSpecHandler(pyxet.XetFS(url_info=CONSTANTS.TITANIC_MAIN)))
    ...
    with pytest.raises(NotImplementedError):  # TODO
        pa_fs.copy_file(CONSTANTS.TITANIC_CSV, 'https://xethub.com/xdssio/titanic.git/main/titanic2.csv')
