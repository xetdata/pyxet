...
def test_single_file_upload():
    ...
    b1 = utils.new_random_branch_from(f"xet://hub.xetsvc.com:{user}/{repo}", "main")
    ...
    try:
        ...
        dest_list = [
            ...
            (f"xet://hub.xetsvc.com:{user}/{repo}/{b1}", [f"xet://hub.xetsvc.com:{user}/{repo}/{b1}/data"]),
            (f"xet://hub.xetsvc.com:{user}/{repo}/{b1}/",[f"xet://hub.xetsvc.com:{user}/{repo}/{b1}/data"]),
            (f"xet://hub.xetsvc.com:{user}/{repo}/{b1}/zz", [f"xet://hub.xetsvc.com:{user}/{repo}/{b1}/zz"]),
        ]
            ...
            for src in source_list:
                for dest in dest_list:
                    for r in recursive_list:
                        ...
                        utils.assert_remote_files_exist(f"xet://hub.xetsvc.com:{user}/{repo}/{b1}/*", dest[1])
                        pyxet.PyxetCLI.rm(dest[1])
        finally:
            shutil.rmtree(dir)
    finally:
        delete_branch(f"xet://hub.xetsvc.com:{user}/{repo}", b1, True)
...
def test_multiple_files_upload():
    ...
    try:
        ...
        dest_list = [
            f"xet://hub.xetsvc.com:{user}/{repo}/{b1}",
            f"xet://hub.xetsvc.com:{user}/{repo}/{b1}/",
        ]
        ...
        expected_files = [f"xet://hub.xetsvc.com:{user}/{repo}/{b1}/data0", f"xet://hub.xetsvc.com:{user}/{repo}/{b1}/data1"]
            ...
            for src in source_list:
                for dest in dest_list:
                    for r in recursive_list:
                        ...
                        utils.assert_remote_files_exist(f"xet://hub.xetsvc.com:{user}/{repo}/{b1}/*", expected_files)
            ...
            for src in source_list:
                for dest in dest_list:
                    for r in recursive_list:
                        ...
                        perform_copy(src, dest, "add data", r)
                        utils.assert_remote_files_exist(f"xet://hub.xetsvc.com:{user}/{repo}/{b1}/*", expected_files)
                        utils.assert_remote_files_not_exist(f"xet://hub.xetsvc.com:{user}/{repo}/{b1}/*", [f"xet://hub.xetsvc.com:{user}/{repo}/{b1}/{sub_dir}"])
                        pyxet.PyxetCLI.rm(expected_files)
        finally:
            shutil.rmtree(dir)
    finally:
...
def test_glob_recursive_upload():
    ...
    try:
        ...
        expected_files_level1 = [f"xet://hub.xetsvc.com:{user}/{repo}/{b1}/data0", f"xet://hub.xetsvc.com:{user}/{repo}/{b1}/data1", f"xet://hub.xetsvc.com:{user}/{repo}/{b1}/{sub_dir}"]
        expected_files_level2 = [f"xet://hub.xetsvc.com:{user}/{repo}/{b1}/{sub_dir}/data"]
            ...
            for src in source_list:
                for dest in dest_list:
                    for r in recursive_list:
                        print(f"xet cp {src} {dest} {r}")
                        perform_copy(src, dest, "add data", r)
                        utils.assert_remote_files_exist(f"xet://hub.xetsvc.com:{user}/{repo}/{b1}/*", expected_files_level1)
                        utils.assert_remote_files_exist(f"xet://hub.xetsvc.com:{user}/{repo}/{b1}/{sub_dir}/*", expected_files_level2)
                ...
                for dest in dest_list:
                    for r in recursive_list:
                        ...
                        if should_succeed:
                            ...
                            utils.assert_remote_files_exist(f"xet://hub.xetsvc.com:{user}/{repo}/{b1}/*", [f"xet://hub.xetsvc.com:{user}/{repo}/{b1}/data"])
...
def _test_directory_recursive_noslash_upload():
    ...
    try:
        ...
        expected_files_level1 = [f"xet://hub.xetsvc.com:{user}/{repo}/{b1}/{dir_name}"]
        expected_files_level2 = [f"xet://hub.xetsvc.com:{user}/{repo}/{b1}/{dir_name}/data0", f"xet://hub.xetsvc.com:{user}/{repo}/{b1}/{dir_name}/data1", f"xet://hub.xetsvc.com:{user}/{repo}/{b1}/{dir_name}/{sub_dir}"]
        expected_files_level3 = [f"xet://hub.xetsvc.com:{user}/{repo}/{b1}/{dir_name}/{sub_dir}/data"]
            ...
            for src in source_list:
                for dest in dest_list:
                    for r in recursive_list:
                        ...
                        perform_copy(src, dest, "add data", r)
                        utils.assert_remote_files_exist(f"xet://hub.xetsvc.com:{user}/{repo}/{b1}/*", expected_files_level1)
                        utils.assert_remote_files_exist(f"xet://hub.xetsvc.com:{user}/{repo}/{b1}/{dir_name}/*", expected_files_level2)
                        utils.assert_remote_files_exist(f"xet://hub.xetsvc.com:{user}/{repo}/{b1}/{dir_name}/{sub_dir}/*", expected_files_level3)
                        pyxet.PyxetCLI.rm(expected_files_level1)
        finally:
            shutil.rmtree(dir)
...
def test_large_batch_upload():
    ...
    try:
        ...
        try:
            ...
            perform_copy(f"{dir}/", f"xet://hub.xetsvc.com:{user}/{repo}/{b1}", "add data", True)
        finally:
            shutil.rmtree(dir)
...
def test_size_hint():
    ...
    try:
        ...
        cplist = build_cp_action_list(f"{dir}/*", f"xet://hub.xetsvc.com:{user}/{repo}/main")
        assert len(cplist) == 2
